				...
				if (data != "null") {				    
					$.each( data, function( key, val ) {
						...
						if (val==="Yes") {
							...
							_gaq.push(['_trackEvent', 'Pop Up', 'Display', 'DHN Facebook', 0, true]);
						   }
						 else if (val==="No") {
						 //alert("Subscribed NO");                                    
						    $.magnificPopup.open({	
							  ...
							  $('head').append('<meta id="viewport" name="viewport" content="width=device-width, initial-scale=.7, maximum-scale=.7, user-scalable=no">');                
								...
								$('head').append('<meta id="viewport" name="viewport" content="width=964">');
                                            ...
                                            _gaq.push(['_trackEvent', 'Pop Up', 'Display', 'DHN Dr Fav', 0, true]);
						 };
					});
				} 				
			});
...
FB.Event.subscribe('edge.create', function(response) {
 _gaq.push(['_trackEvent', 'Pop Up', 'Like', 'DHN Facebook', 0, true]);
writeCookie('nevershow');
});
FB.Event.subscribe('edge.remove', function(response) {
 _gaq.push(['_trackEvent', 'Pop Up', 'Unlike', 'DHN Facebook', 0, true]);
});
  };
...
_gaq.push(['_trackEvent', 'Pop Up', 'Decline', 'DHN Dr Fav', 0, true]);
e.preventDefault();
$.magnificPopup.close();
});

$(document).on('click', '.popup-modal-dismiss-facebook', function (e) {
_gaq.push(['_trackEvent', 'Pop Up', 'Decline', 'DHN Facebook', 0, true]);
e.preventDefault();
$.magnificPopup.close();
});

$(document).on('click', '.facebook', function (e) {
_gaq.push(['_trackEvent', 'Pop Up', 'Close', 'DHN Facebook', 0, true]);
e.preventDefault();
...
_gaq.push(['_trackEvent', 'Pop Up', 'Close', 'DHN Dr Fav', 0, true]);
e.preventDefault();
writeCookie('nevershow', 'oneday');
$.magnificPopup.close();
});
...
jQuery(document).ready(function($){
	$('.submit').submit(function() {
		...
		if (emailval) {
                        var nl = "DHN";
                        var source = 'Modal Pop-up DHN Dr Fav';
                        //var mid = 44556845 BLS;
                        var mid = 44798890; 
                        var url = location.protocol + '//' + location.host + '/popup/' + 'sp_user_update.php?email=' + emailval + '&nl=' + nl + '&source=' + source + '&mid=' + mid;
                        ...
                        $.getJSON(url, function(data) {
                                    if (data != "null") {
                                                if (data == "true") {
															...
															_gaq.push(['_trackEvent', 'Pop Up', 'Opt In', 'DHN Dr Fav', 0, true]);
